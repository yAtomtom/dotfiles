...
- プランモード中は実行不可: 呼び出し元がプランモード中の場合、「プラン承認後に `/cross-review` で実行してください」と報告して終了する
...
```bash
command copilot --model "gpt-5.3-codex" --effort "xhigh" --add-dir $HOME/claude-sandbox/ -p "~/claude-sandbox/<対象ファイル名> をレビューしてください"
```
